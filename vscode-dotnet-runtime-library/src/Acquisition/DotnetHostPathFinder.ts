...
export class DotnetHostPathFinder implements IDotnetPathFinder
{

    public constructor(private readonly workerContext: IAcquisitionWorkerContext, private readonly utilityContext: IUtilityContext, private executor?: ICommandExecutor, private file?: IFileUtilities, private resolver?: DotnetResolver)
    {
        this.executor ??= new CommandExecutor(this.workerContext, this.utilityContext);
        this.file ??= new FileUtilities();
        this.resolver ??= new DotnetResolver(workerContext, utilityContext, this.executor);
    }
     ...
     */
    public async findDotnetRootPath(suggestedArchitecture: string): Promise<string | undefined>
    {
        this.workerContext.eventStream.post(new DotnetFindPathLookupRootPATH(`Looking up .NET on the root.`));

        if (suggestedArchitecture === 'x64' && (this.executor !== undefined ? (await getOSArch(this.executor)).includes('arm') : false))
        {
     ...
     * @param suggestedArchitecture - The architecture to suggest to the resolver, or null if not applicable - must be validated.
     * @returns The path to the dotnet executable, which may be a symlink to the actual executable.
     */
    public async findDotnetFastFromListOnly(suggestedArchitecture: string | null): Promise<string | null>
    {
        const finalPath = await this.resolver!.resolveTruePath('dotnet', suggestedArchitecture);
        return finalPath;
    }
     ...
     */
    public async findRawPathEnvironmentSetting(tryUseTrueShell = true, suggestedArchitecture: string | null): Promise<string[] | undefined>
    {
        ...
        if (dotnetsOnPATH && (dotnetsOnPATH?.length ?? 0) > 0)
        {
            ...
            return this.resolver?.resolveTruePaths(dotnetsOnPATH, suggestedArchitecture);
        }
        else
        {
     ...
     */
    public async findRealPathEnvironmentSetting(tryUseTrueShell = true, suggestedArchitecture: string | null): Promise<string[] | undefined>
    {
        this.workerContext.eventStream.post(new DotnetFindPathLookupRealPATH(`Looking up.NET on the real path.`));
        const dotnetsOnPATH = await this.findRawPathEnvironmentSetting(tryUseTrueShell, suggestedArchitecture);
        const realPaths = [];
        ...
        return this.resolver?.resolveTruePaths(realPaths, suggestedArchitecture);
    }
