...
export class LinuxVersionResolver
{
    ...
    private DistroProviderFactory(distroAndVersion : DistroVersionPair) : IDistroDotnetSDKProvider
    {
        switch(distroAndVersion.distro)
        {
            ...
            case null:
                const unknownDistroErr = new DotnetAcquisitionDistroUnknownError(new Error(this.baseUnsupportedDistroErrorMessage), getInstallKeyFromContext(this.acquireCtx));
